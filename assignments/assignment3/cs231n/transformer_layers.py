...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        div_term = torch.exp(-math.log(10000)*torch.arange(0,embed_dim,2) / embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # 2. Split heads
        # 维度变换步骤: (N, S, E) -> (N, S, H, d_k) -> (N, H, S, d_k)
        # 使用 view 而不是 reshape 效率更高，permute 用于交换多个维度
        q = q.view(N,S,H,d_k).permute(0,2,1,3)
        k = k.view(N,T,H,d_k).permute(0,2,1,3)
        v = v.view(N,T,H,d_k).permute(0,2,1,3)

        # 3. 计算注意力分数 (Scaled Dot-Product)
        # q: (N, H, S, d_k), k.transpose: (N, H, d_k, T) -> scores: (N, H, S, T)        
        scores = torch.matmul(q,k.transpose(-2,-1)) / math.sqrt(d_k)

        # 4. 应用 Mask
        if attn_mask is not None:
            # 假设 mask 形状是 (S, T)，由于 scores 是 (N, H, S, T)，PyTorch 会自动广播
            # mask == 0 的地方填充 -inf
        ...
        # 6. 加权求和 (Attention context)
        # (N, H, S, T) @ (N, H, T, d_k) -> (N, H, S, d_k)
        context = torch.matmul(weights,v)

        # 7. 合并多头 (Combine heads)
        # (N, H, S, d_k) -> (N, S, H, d_k) -> (N, S, E)
        # .contiguous() 是必须的，因为 permute 改变了内存步长
        output = context.permute(0,2,1,3).contiguous().view(N,S,E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = torch.reshape(x,(N,C,H//patch_size,patch_size,W//patch_size,patch_size))
        patches_permute = torch.permute(patches,(0,2,4,1,3,5)).contiguous().reshape(N,num_patches,patch_dim)
        out = self.proj(patches_permute)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return out
