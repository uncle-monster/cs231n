...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        denominator_i_j, denominator_j_i = 0, 0
        for r in range(2*N):  
            if r != k:
                denominator_i_j += torch.exp(sim(z_k,out[r])/tau)
            if r != (k+N):
                denominator_j_i += torch.exp(sim(z_k_N,out[r])/tau)
        pos_sim = torch.exp(sim(z_k,z_k_N)/tau)
        l_i_j = -torch.log(pos_sim / denominator_i_j)
        l_j_i = -torch.log(pos_sim / denominator_j_i)
        total_loss += l_i_j +l_j_i
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    loss = None
    loss = torch.sum(-torch.log(numerator / (denom + 1e-8)), dim=0) / (2*N)
