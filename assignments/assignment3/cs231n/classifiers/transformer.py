...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        # 1. 文本处理：Embedding + Positional Encoding
        # 注意：这里的 positional_encoding 内部通常已经包含了加法和 dropout
        ...
        # 4. 进入 Transformer Decoder
        # tgt_mask 确保每个单词只能看到之前的单词
        output = self.transformer(tgt, memory, tgt_mask=mask) # (N, T, W)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # # Final classification layer to predict class scores from pooled token.
        # self.head = nn.Linear(embed_dim, num_classes)
        src = self.patch_embed(x)
        pe = self.positional_encoding(src)
        out = self.transformer(pe)
